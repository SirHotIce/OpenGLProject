...
void GLpipeline::ObjectManager::PrimeObjectIndexed() {
    glGenVertexArrays(1, &VAO);
    glBindVertexArray(VAO);
...
void GLpipeline::ObjectManager::DrawObjectIndexed( CameraManager& camera, std::vector<Light>& lights, Material& material) {
    glUseProgram(shaderProgram);
    ...
    glUniform1i(texLoc, 0);//telling the shader that the value for the uniform tex loc is 0
        glBindVertexArray(VAO);
                glDrawElements(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, 0);//no need to pass indices here as we have an already going IBO bound
        glBindVertexArray(0);
